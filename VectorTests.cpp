...
void VectorTests::TestBinarySearchForRawMultiple()
{
	...
	const int invArrInvalid[numElems]         = { 1, 4, 100 };
	...
	Assert(vInt.binary_search(invArrInvalid, numElems) == false, "test_2");
...
{
	unsigned int reservedLength = 70;
	unsigned int actualLength = (unsigned int)strlen(str);
